...
export function PlayerDashboard() {
  ...
  const handleSubtitleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setLocalSubtitleSrc(URL.createObjectURL(file));
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F3F4F6] dark:bg-[#050505] text-[#111827] dark:text-[#EDEDED] antialiased transition-colors duration-200">
      {/* Header */}
      <header className="w-full h-14 flex items-center justify-between px-6 border-b border-[#E5E7EB] dark:border-[#1F1F23] bg-[#FFFFFF] dark:bg-[#0A0A0A] transition-colors duration-200 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <div className="w-4 h-4 bg-[#111111] dark:bg-white rounded-full"></div>
          <h1 className="text-sm font-bold tracking-tight uppercase">
            {roomState?.name || "Sync Room"}
          </h1>
          <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-widest border border-green-200 dark:border-green-800">
            Direct Live
          </span>
        </div>

        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-[11px] font-semibold text-[#6B7280] dark:text-[#A1A1AA] uppercase tracking-wider">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
            Connection Stable
          </div>
          <div className="h-4 w-px bg-[#E5E7EB] dark:bg-[#1F1F23] mx-1"></div>
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto p-6 space-y-6">
        {/* Local Media Picker */}
        <section className="bg-white dark:bg-[#0A0A0A] border border-[#E5E7EB] dark:border-[#1F1F23] rounded-xl p-4 shadow-sm flex items-center gap-6">
          ...
          <div className="h-10 w-px bg-[#E5E7EB] dark:bg-[#1F1F23]"></div>
          <div className="flex-1">
            <label className="block text-xs font-bold text-[#111827] dark:text-[#EDEDED] uppercase tracking-wider mb-2">
        ...
        <section className="w-full">
          <div className="bg-black rounded-xl overflow-hidden shadow-2xl relative border border-[#E5E7EB] dark:border-[#1F1F23] aspect-video max-h-[70vh] flex flex-col mx-auto cursor-default">
            ...
            <div className="absolute top-6 left-6 flex gap-3 z-20 pointer-events-none">
              <span className="bg-red-600 text-white text-[11px] font-bold px-3 py-1 rounded-full flex items-center gap-1.5 shadow-lg border border-red-500/50">
                LIVE
              </span>
              <span className="bg-black/60 text-white text-[11px] font-medium px-3 py-1 rounded-full backdrop-blur-md border border-white/10">
                1080p · 60 FPS
              </span>
            </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-xs font-bold font-mono text-[#111827] dark:text-[#EDEDED]">
                      {new Date((roomState?.currentTime || 0) * 1000)
                        .toISOString()
                        .substr(11, 8)}
                    </span>
                    <div className="w-64 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden cursor-pointer">
                      <div
                        className="h-full w-[85%] bg-[#111111] dark:bg-white"
                        style={{ width: "85%" /* mock logic */ }}
                      ></div>
                    </div>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-12">
          ...
          <div className="lg:col-span-4 bg-[#FFFFFF] dark:bg-[#0A0A0A] border border-[#E5E7EB] dark:border-[#1F1F23] rounded-xl p-5 shadow-sm flex flex-col h-[280px]">
          ...
          <div className="lg:col-span-8 bg-[#FFFFFF] dark:bg-[#0A0A0A] border border-[#E5E7EB] dark:border-[#1F1F23] rounded-xl shadow-sm overflow-hidden flex flex-col h-[280px]">
            <div className="px-5 py-4 border-b border-[#E5E7EB] dark:border-[#1F1F23] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                <h3 className="text-xs font-bold text-[#111827] dark:text-[#EDEDED] uppercase tracking-wider">
                  Active Members
                </h3>
                    ...
                    activeUsers.map((user) => (
                      ...
                      >
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-full ${user.avatar || "bg-gradient-to-br from-indigo-400 to-purple-500"} flex items-center justify-center text-white text-[10px] font-black border border-white/20`}
                            >
                        ...
                        <td className="px-5 py-3 text-right">
                          <span className="text-[10px] font-bold font-mono text-[#111827] dark:text-[#EDEDED]">
                            {/* Uses actual recorded latency if they are the current user connecting, otherwise assuming < 50ms since other users latency isn't currently broadcast via DO architecturally */}
                            {user.id === currentUser?.id
                              ? `${latency}ms`
                              : `< 50ms`}
                          </span>
